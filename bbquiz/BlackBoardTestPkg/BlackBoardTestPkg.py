...
class Pool(BlackBoardObject):
    ...
    def close(self):
        ref = self.package.embed_resource(
            self.pool_name, "assessment/x-bb-qti-pool",
    ...
    def addNumQ(self, title, text, answer, errfrac=None, erramt=None,
                errlow=None, errhigh=None,
        ...
        response_num = etree.SubElement(flow2, 'response_num',
                                        {'ident':'response',
                                         'rcardinality':'Single',
                                         'rtiming':'No'})
        etree.SubElement(response_num, 'render_fib',
                         {'charset':'us-ascii',
                          'encoding':'UTF_8',
                          'rows':'0',
                          'columns':'0',
    ...
    def addMCQ(self, title, text, answers, correct=0,
               positive_feedback="Good work",
               negative_feedback="That's not correct",
               shuffle_ans=True):
        ...
        flow2 = etree.SubElement(flow1, 'flow', {'class':'RESPONSE_BLOCK'})
        response_lid = etree.SubElement(flow2, 'response_lid',
                                        {'ident':'response',
                                         'rcardinality':'Single',
                                         'rtiming':'No'})
        render_choice = etree.SubElement(response_lid,
                                         'render_choice',
                                         {'shuffle':'Yes' if
                                          shuffle_ans else 'No',
                                          'minnumber':'0',
        ...
        for idx,text in enumerate(answers):
            flow_label = etree.SubElement(render_choice, 'flow_label',
                                          {'class':'Block'})
            a_uuids.append(uuid.uuid4().hex)
            response_label = etree.SubElement(flow_label,
                                              'response_label',
                                              {'ident':a_uuids[-1],
                                               'shuffle':'Yes',
                                               'rarea':'Ellipse',
