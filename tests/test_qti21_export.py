...
"""
    ...
    pkg_template_dir = Path(__file__).parent.parent / "src" / "quizml" / "templates" / "qti21"
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        resources = manifest_root.findall(".//{http://www.imsglobal.org/xsd/imscp_v1p1}resource")
        # 1 test resource + 8 item resources
        assert len(resources) == 9

        # Ensure imsqti_item_xmlv2p1 type is used for all items
        item_res = [r for r in resources if r.attrib.get("type") == "imsqti_item_xmlv2p1"]
        assert len(item_res) == 8

        test_res = [r for r in resources if r.attrib.get("type") == "imsqti_test_xmlv2p1"]
        ...
        item_refs = assessment_root.findall(".//{http://www.imsglobal.org/xsd/imsqti_v2p1}assessmentItemRef")
        ...
        choice_interaction = q1_root.find(".//{http://www.imsglobal.org/xsd/imsqti_v2p1}choiceInteraction")
        assert choice_interaction is not None
        assert choice_interaction.attrib.get("maxChoices") == "1"
        assert choice_interaction.attrib.get("shuffle") == "false"  # Explicitly set false in question

        # Correct response is choice_1 ("4")
        correct_val = q1_root.find(".//{http://www.imsglobal.org/xsd/imsqti_v2p1}correctResponse/{http://www.imsglobal.org/xsd/imsqti_v2p1}value")
        ...
        ma_interaction = q2_root.find(".//{http://www.imsglobal.org/xsd/imsqti_v2p1}choiceInteraction")
        assert ma_interaction is not None
        assert ma_interaction.attrib.get("maxChoices") == "0"  # Unlimited choices for MA
        correct_vals = [v.text for v in q2_root.findall(".//{http://www.imsglobal.org/xsd/imsqti_v2p1}correctResponse/{http://www.imsglobal.org/xsd/imsqti_v2p1}value")]
        assert correct_vals == ["choice_0", "choice_1", "choice_3"]  # 2, 3, 5 are primes

        # 5. Verify item 3 (TF)
        q3_raw = zf.read("items/item_3.xml")
        q3_root = ET.fromstring(q3_raw)
        tf_interaction = q3_root.find(".//{http://www.imsglobal.org/xsd/imsqti_v2p1}choiceInteraction")
        assert tf_interaction is not None
        assert tf_interaction.attrib.get("maxChoices") == "1"
        tf_val = q3_root.find(".//{http://www.imsglobal.org/xsd/imsqti_v2p1}correctResponse/{http://www.imsglobal.org/xsd/imsqti_v2p1}value")
        ...
        essay_interaction = q6_root.find(".//{http://www.imsglobal.org/xsd/imsqti_v2p1}extendedTextInteraction")
...
def test_qti21_export_with_image_and_html_tables(tmp_path):
    ...
    with zipfile.ZipFile(out_zip, "r") as zf:
        ...
        # Check item 2 has embedded base64 image
        q2_xml = zf.read("items/item_2.xml").decode("utf-8")
        assert "data:image" in q2_xml
