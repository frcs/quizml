...
# class ImageAttr(inline.InlineElement):
#     elements = [ImageAttr]
#     renderer_mixins = [ImageAttrRendererMixin]


class ImageAttr(inline.InlineElement):
    pattern = r'(?:[!]\[(?P<caption>.*?)\])\((?P<image>.*?)\)\{(?P<attr>.*?)\}'
    parse_children = True
    priority = 6

    def __init__(self, match):
        self.target = match.group(2)
        self.attributes = match.group(3)
        print("AHARAR")
        
class ImageAttrRendererMixin(object):
    def render_image_attr(self, element):
        return '\includegraphics[{}]{{{}}}'.format(
            element.attributes, element.target
        )
        # return '<a ref="{}" {}>{}</a>'.format(
        #     element.target, element.attributes, self.render_children(element)
        # )
...
#         ImageAttrMixin.arg = arg
#         self.elements = [ImageAttr]
#         self.renderer_mixins = [ImageAttrRendererMixin]


mdtext = """
In this paper we consider the case where $\\frac{1}{2}\\int_x \\cos(x) dx = \\phi$

This is an intersting problem, *with*:
...
An an image below:
![Caption: A picture](test.png){width=30em}
...
markdown = marko.Markdown(renderer=LatexRenderer,extensions=[ImageAttr,GitHubWiki])

print(markdown.convert(mdtext ))
