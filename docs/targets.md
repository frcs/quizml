...
* **Moodle XML (`moodle`):** Produces a self-contained `.xml` file conforming to the native **Moodle XML format**. Suitable for importing directly into Moodle courses via the *Question bank* $\to$ *Import* $\to$ *Moodle XML format* menu. Supports all QuizML question types, embedded MathML equations, and base64 images.
